...
# morphofit imports
from morphofit.catalogue_managing import assign_sources_to_region, get_best_fit_parameters_from_model_image
from morphofit.utils import get_logger, compress_and_copy_files_for_galfit_regions
...
from morphofit.utils import save_best_fit_properties_regions, save_galfit_output_files
...
def galfit_on_regions(args, region_index, telescope_name, target_field_name, waveband, sci_image_region_filename,
                      seg_image_region_filename, exposure_time, magnitude_zeropoint, instrumental_gain, pixel_scale,
                      ...
                      best_fit_properties_table_filename, rms_image_region_filename=None,
                      exp_image_region_filename=None, constraints_file_filename='None',
    ...
    :param waveband:
    :param sci_image_region_filename:
    :param seg_image_region_filename:
    :param exposure_time:
    ...
    :param rms_image_region_filename:
    :param exp_image_region_filename:
    ...
    logger.info('=============================== {}, {}, region {}, {}, {}, {}'.format(target_field_name, waveband,
                                                                                       region_index,
                                                                                       psf_image_type,
                                                                                       sigma_image_type,
                                                                                       background_estimate_method))

    logger.info('=============================== assign galaxies to region')
    source_galaxies_keys = args.source_galaxies_keys.split(',')
    region_catalogue = assign_sources_to_region(sci_image_region_filename, source_galaxies_catalogue,
                                                ra_keyword=source_galaxies_keys[1],
                                                dec_keyword=source_galaxies_keys[2],
                                                waveband_keyword=args.source_galaxies_reference_waveband)

    logger.info('=============================== create bad pixel mask for GALFIT')
    bad_pixel_mask_filename = create_bad_pixel_mask_for_galfit(region_catalogue, seg_image_region_filename,
                                                               id_key_sources_catalogue=source_galaxies_keys[0])

    logger.info('=============================== local background estimate in region')
    local_background_value = local_background_estimate(sci_image_region_filename, seg_image_region_filename,
    ...
    magnitude_zeropoint, local_background_value = create_sigma_image_for_galfit(telescope_name,
                                                                                ...
                                                                                sci_image_region_filename,
                                                                                rms_image_region_filename,
    ...
    light_profiles, source_positions, ra, dec, total_magnitudes, effective_radii, sersic_indices, axis_ratios, \
        position_angles, subtract = format_properties_for_galfit(sci_image_region_filename, region_catalogue,
    ...
    header = fits.getheader(sci_image_region_filename)
    image_size_x = int(header['NAXIS1'])
    image_size_y = int(header['NAXIS2'])
    image_size = [image_size_x, image_size_y]

    logger.info('=============================== create GALFIT input file')
    create_galfit_inputfile(input_galfit_filename, os.path.basename(sci_image_region_filename),
    ...
    save_best_fit_properties_regions(best_fit_properties_table_filename, region_catalogue,
                                     target_field_name, region_index, waveband, telescope_name, source_galaxies_keys[0],
    ...
    save_galfit_output_files(output_directory, sci_image_region_filename, input_galfit_filename,
...
def main(indices, args):
    ...
    for index in indices:
        ...
        sci_image_region_filename = os.path.join(temp_dir,
                                                 h5table['sci_image_region_filenames'][()][index].decode('utf8'))
        rms_image_region_filename = os.path.join(temp_dir,
                                                 h5table['rms_image_region_filenames'][()][index].decode('utf8'))
        exp_image_region_filename = os.path.join(temp_dir,
                                                 h5table['exp_image_region_filenames'][()][index].decode('utf8'))
        seg_image_region_filename = os.path.join(temp_dir,
                                                 h5table['seg_image_region_filenames'][()][index].decode('utf8'))
        ...
        rms_image_filename, exp_image_filename, constraints_file_filename = \
            compress_and_copy_files_for_galfit_regions(index, root_target_field, temp_dir, args.files_archive_prefix,
                                                       sci_image_region_filename, rms_image_region_filename,
                                                       exp_image_region_filename,
                                                       seg_image_region_filename, source_galaxies_catalogue_filename,
        ...
        region_index = h5table['region_indices'][()][index].decode('utf8')
        ...
        best_fit_properties_table_filename = os.path.join(temp_dir, '{}_{}_{}_{}_{}_{}_{}.fits'
                                                          .format(telescope_name,
                                                                  target_field_name,
                                                                  waveband, region_index,
                                                                  psf_image_type,
                                                                  sigma_image_type,
                                                                  background_estimate_method))

        os.makedirs(os.path.join(root_target_field,
                                 'regions/region{}_{}_{}_{}'.format(region_index, psf_image_type, sigma_image_type,
                                                                    background_estimate_method)), exist_ok=True)
        output_dir = os.path.join(root_target_field,
                                  'regions/region{}_{}_{}_{}'.format(region_index, psf_image_type, sigma_image_type,
                                                                     background_estimate_method))

        galfit_on_regions(args, region_index, telescope_name, target_field_name, waveband, sci_image_region_filename,
                          seg_image_region_filename, exposure_time, magnitude_zeropoint, instrumental_gain,
                          ...
                          psf_image_filename, psf_sampling_factor, convolution_box_size, galfit_binary_file,
                          best_fit_properties_table_filename, rms_image_region_filename=rms_image_filename,
                          exp_image_region_filename=exp_image_filename,
...
def check_missing(indices, args):
    ...
    for index in indices:
        ...
        root_target_field = h5table['root_target_fields'][()][index].decode('utf8')
        region_index = h5table['region_indices'][()][index]
        sigma_image_type = h5table['sigma_image_types'][()][index].decode('utf8')
        background_estimate_method = h5table['background_estimate_methods'][()][index].decode('utf8')
        psf_image_type = h5table['psf_image_types'][()][index].decode('utf8')

        output_directory = os.path.join(root_target_field,
                                        'regions/region{}_{}_{}_{}'.format(region_index, psf_image_type,
                                                                           sigma_image_type,
                                                                           background_estimate_method))
...
def setup(args):
    ...
    description = "Run GALFIT on regions containing target galaxies in Target Fields"
